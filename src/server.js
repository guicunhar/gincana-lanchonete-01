...
io.on('connection', (socket) => {
  ...
  socket.on('host:start', (_, cb) => {
    ...
    if (Object.keys(room.players).length < GAME_CONFIG.minPlayers) {
      return cb?.({ error: `Aguarde pelo menos ${GAME_CONFIG.minPlayers} jogador(es).` });
    }
  ...
  socket.on('player:join', ({ code, name }, cb) => {
    ...
    io.to(result.room.code).emit('room:playerList', getRoomPlayerList(result.room));
  ...
  socket.on('player:answer', ({ questionId, assignments }, cb) => {
    ...
    if (!room || room.status !== 'question') return cb?.({ error: 'Não há pergunta ativa.' });
    ...
    const answered = Object.values(room.players).filter(p => p.answers[questionId]).length;

    // Notify host of progress
    io.to(room.hostSocketId).emit('host:answerProgress', { answered, total: totalPlayers });
  ...
  socket.on('disconnect', () => {
    ...
    if (result.type === 'host_left') {
      ...
      io.to(result.code).emit('room:playerList', getRoomPlayerList(result.room));
...
function advanceToNextQuestion(room) {
  ...
  console.log(`[Q] ${room.code} question ${questionIndex + 1}/${totalQuestions}`);
