/* ─────────────────────────────────────────────────────────────
   GINCANA — Frontend App
   ───────────────────────────────────────────────────────────── */

const socket = io();

// Reconexão automática ao reabrir a página
socket.on("connect", () => {
  const role = sessionStorage.getItem("qm_role");
  const code = sessionStorage.getItem("qm_host_code");
  if (role === "host" && code) {
    socket.emit("host:rejoin", { code }, (res) => {
      if (res?.ok) {
        ...
        document.getElementById("btn-start").disabled = res.playerCount < 1;
        document.getElementById("start-hint").textContent =
          res.playerCount > 0
            ? `${res.playerCount} jogador(es) conectado(s)`
            : "Aguardando jogadores...";
        showScreen("screen-host-lobby");
        toast("Reconectado à sala!", "success");
      } else {
        // Sala já expirou, limpa
        sessionStorage.removeItem("qm_host_code");
...
const App = (() => {
  ...
  function createRoom() {
    ...
    socket.emit("host:create", { hostName: name }, (res) => {
      ...
      // Salva no sessionStorage pra sobreviver troca de aba
      sessionStorage.setItem("qm_host_code", res.code);
      sessionStorage.setItem("qm_role", "host");
  ...
  function joinRoom() {
    ...
    if (!code || code.length < 4)
      return toast("Digite o código da sala.", "error");
  ...
  function startTimer(seconds, barId, valueId) {
    ...
    function tick() {
      ...
      if (bar)
        bar.className = "timer-bar" + (remaining <= 10 ? " warning" : "");
  ...
  function renderQuestion(data) {
    ...
    // Meta
    document.getElementById("q-meta").textContent = `Pergunta ${
      questionIndex + 1
    } de ${totalQuestions}`;
    ...
    if (inCategory) {
      ...
      for (const catId of Object.keys(state.assignments)) {
        ...
        if (idx !== -1) {
          ...
          const catChip = document.querySelector(
            `#cat-items-${catId} [data-item="${item}"]`
          );
  ...
  function handleCategoryClick(cat) {
    ...
    const poolChip = document.querySelector(
      `#items-pool [data-item="${item}"]`
    );
    if (poolChip) {
  ...
  function clearSelection() {
    document
      .querySelectorAll(".item-chip.selected")
  ...
  function renderScoreboard(data) {
    ...
    document.getElementById("scoreboard-subtitle").textContent = isLast
      ? "Última pergunta!"
      : "Placar parcial";
    ...
    if (state.role === "player") {
      const myScore = Object.values(scores).find(
        (s) => s.name === state.playerName
      );
      if (myScore) {
        ...
        if (q && q.categories) {
          q.categories.forEach((cat) => {
            ...
            catResult.chosen.forEach((item) => {
              ...
              chip.className =
                "review-item " + (correctSet.has(item) ? "hit" : "miss");
              chip.textContent = item;
    ...
    // Host controls
    document.getElementById("host-next-wrap").style.display =
      state.role === "host" ? "" : "none";
    document.getElementById("waiting-next").style.display =
      state.role === "player" ? "" : "none";
    if (state.role === "host") {
  ...
  socket.on("room:playerList", (players) => {
    ...
    if (state.role === "host") {
      ...
      document.getElementById("start-hint").textContent =
        players.length >= 1
          ? `${players.length} jogador(es) pronto(s)!`
          : "Aguardando jogadores...";
    }
  });
  ...
  socket.on("game:question", (data) => {
    if (state.role === "host") {
      ...
      document.getElementById("host-q-meta").textContent = `Pergunta ${
        data.questionIndex + 1
      } de ${data.totalQuestions}`;
